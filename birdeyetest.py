...
def get_frame():
   # camera_port=1 #for sep camera
    camera_port = getNum(os.readlink("/dev/ARC_International_CAM")) #getNum(os.readlink("/dev/LOGITECH_C310_TOP"))
    print(camera_port)
    cap=cv2.VideoCapture(camera_port) #this makes a web cam object

    cap.set(cv2.cv.CV_CAP_PROP_FRAME_HEIGHT,160)
...
if __name__ == '__main__':
    app.run(host='10.8.46.177', debug=True, threaded=True)
